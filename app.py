...
def create_signal_alert(tag, matching_posts, citizens=None):
    ...
    def _oracle():
        try:
            _t.sleep(2)
            full = db.get_post(alert.get('id',''))
            if full:
                brief = ORACLE.synthesise(full)
                if brief:
                    db.save_brief(brief)
                    log.info(f"ORACLE brief: {brief['headline']}")
        except Exception as e:
            log.error(f"ORACLE auto-synthesis failed: {e}")
    threading.Thread(target=_oracle, daemon=True).start()
...
def setup_scheduler():
    ...
    scheduler.add_job(lambda: run_agent('VIGIL'),   'interval', hours=2,     id='vigil')
    scheduler.add_job(lambda: run_agent('LORE'),    'interval', hours=3,     id='lore')
    scheduler.add_job(lambda: run_agent('SPECTER'), 'interval', minutes=75,  id='specter')
    scheduler.add_job(lambda: COUNCIL.run_on_unprocessed(db), 'interval', minutes=90, id='council')
    scheduler.add_job(lambda: ORACLE.run_on_unprocessed(db),  'interval', hours=2,    id='oracle')
    scheduler.start()
    log.info("Scheduler started — 12 agents + COUNCIL + ORACLE active")
    return scheduler
