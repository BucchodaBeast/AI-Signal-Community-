...
"""
    ...
    def _fetch_federal_register(self):
        ...
        try:
            # Federal Register requires bracket-notation params as tuples
            params = [
    ...
    def _fetch_usaspending(self):
        """USASpending v2 API — federal contract awards. Short timeout — never blocks the run."""
        ...
        try:
            resp = requests.post(
                ...
                timeout=10,  # Reduced: skip gracefully if slow, Federal Register covers this run
            )
            if not resp.ok:
                self.log.warning(f"USASpending {resp.status_code} — skipping, Federal Register will cover")
                return []
            results = resp.json().get('results', [])
            random.shuffle(results)
            return [{
                'source':      'USASpending',
                'id':          r.get('Award ID') or str(i),
            ...
            return []  # Federal Register always covers

    def _fetch_regulations_gov(self):
        """Congress.gov EFTS search — free, no API key, replaces regulations.gov DEMO_KEY.
        DEMO_KEY is shared globally and hits its 1000 req/day limit within hours.
        """
        topics = [
        ...
        try:
            ...
            items = [{
                ...
                'docket_type': r.get('collectionCode', ''),
                'modified':   r.get('dateIssued', ''),
                'highlights': (r.get('snippet') or '')[:200],
                'topic':      topic,
            } for r in results if r.get('title')]
            return items if items else self._fetch_federal_register()
        except Exception as e:
