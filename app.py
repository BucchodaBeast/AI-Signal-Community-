...
def setup_scheduler():
    ...
    scheduler.add_job(lambda: run_agent('VIGIL'),   'interval', hours=2,     id='vigil')   # Supply chain moves slowly
    scheduler.add_job(lambda: run_agent('LORE'),    'interval', hours=3,     id='lore')    # Patents are slow signals
    scheduler.add_job(lambda: run_agent('SPECTER'), 'interval', minutes=75,  id='specter') # Breaches surface unpredictably
    scheduler.add_job(lambda: ORACLE.run_on_unprocessed(db), 'interval', hours=2, id='oracle')
    scheduler.start()
    log.info("Scheduler started — 12 agents + ORACLE active")
    return scheduler
