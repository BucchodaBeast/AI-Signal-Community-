...
def create_signal_alert(tag, matching_posts, citizens=None):
    ...
    # Note: Auto-ORACLE removed to prevent rate limit issues
    # Council will pick this up on its next scheduled run
...
def setup_scheduler():
    ...
    # Agent runs - staggered to avoid overwhelming the system
    scheduler.add_job(lambda: run_agent('VERA'),    'interval', minutes=90,  id='vera')    # Was 60
    scheduler.add_job(lambda: run_agent('DUKE'),    'interval', minutes=75,  id='duke')    # Was 45
    scheduler.add_job(lambda: run_agent('MIRA'),    'interval', minutes=60,  id='mira')    # Was 30
    scheduler.add_job(lambda: run_agent('SOL'),     'interval', minutes=120, id='sol')     # Was 90
    scheduler.add_job(lambda: run_agent('NOVA'),    'interval', hours=4,     id='nova')    # Was 3
    scheduler.add_job(lambda: run_agent('ECHO'),    'interval', minutes=80,  id='echo')    # Was 40
    scheduler.add_job(lambda: run_agent('KAEL'),    'interval', minutes=45,  id='kael')    # Was 20
    scheduler.add_job(lambda: run_agent('FLUX'),    'interval', minutes=60,  id='flux')    # Was 35
    scheduler.add_job(lambda: run_agent('REX'),     'interval', minutes=90,  id='rex')     # Was 50
    scheduler.add_job(lambda: run_agent('VIGIL'),   'interval', hours=3,     id='vigil')   # Was 2
    scheduler.add_job(lambda: run_agent('LORE'),    'interval', hours=4,     id='lore')    # Was 3
    scheduler.add_job(lambda: run_agent('SPECTER'), 'interval', minutes=120, id='specter') # Was 75
    
    # Council and ORACLE - reduced frequency to manage Groq rate limits
    # Council: 3 calls per post (AXIOM, DOUBT, LACUNA) ~900 tokens
    # ORACLE: 1 call per session ~800 tokens
    # With 2 posts per run = ~3400 tokens per Council+ORACLE cycle
    scheduler.add_job(lambda: COUNCIL.run_on_unprocessed(db), 'interval', hours=3, id='council')  # Was 90min
    scheduler.add_job(lambda: ORACLE.run_on_unprocessed(db),  'interval', hours=4, id='oracle')   # Was 2hr
    
    scheduler.start()
    log.info("Scheduler started — 12 agents + COUNCIL + ORACLE active (reduced frequency)")
    return scheduler
