...
"""
    ...
    def _fetch_fred_rates(self):
        """
        FRED via stlouisfed API v1 JSON endpoint.
        Timeout reduced 10s→6s — fail fast, don't block the run.
        Pick 1 series not 3 — 3×timeout was blocking FLUX for 50s on slow days.
        Falls back to open.er-api.com exchange rates if FRED is unresponsive.
        """
        series_options = [
        ...
        try:
            resp = requests.get(
                ...
                params={'id': sid},
                timeout=6,  # was 10 — fail fast, don't block
            )
            if not resp.ok:
                return self._fetch_rates_fallback()
            ...
            self.log.warning(f'FRED ({sid}): {e} — using fallback')
            return self._fetch_rates_fallback()

    def _fetch_rates_fallback(self):
        """Exchange rates via open.er-api.com — free, no key, fast, on allowed domains."""
        try:
            resp = requests.get(
            ...
            pairs = [('EUR','Euro'),('GBP','British Pound'),('JPY','Japanese Yen'),
                     ('CNY','Chinese Yuan'),('CHF','Swiss Franc')]
            from datetime import datetime as _dt
            today = _dt.utcnow().strftime('%Y-%m-%d')
            items = []
            for code, cname in pairs[:2]:
                rate = rates.get(code, 0)
                if not rate:
                    continue
                items.append({
                    'source': 'ExchangeRate-API', 'id': f'fx-{code}-{today}',
                    'series': f'USD/{code} ({cname})', 'series_id': f'USD{code}',
                    'latest_val': str(rate), 'latest_date': today,
                    'prev_val': '', 'prev_date': '',
                    'change': 0,
                })
            return items
        except Exception as e:
            self.log.error(f'Rates fallback: {e}')
            return []
