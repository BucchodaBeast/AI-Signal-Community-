...
"""
    ...
    def _fetch_open_meteo(self):
        """Open-Meteo — fetch cities one at a time."""
        import time as t
        cities = [
        ...
        for city in cities:
            try:
                ...
                if resp.status_code == 429:
                    self.log.warning("Open-Meteo rate limited, stopping")
                    break
                resp.raise_for_status()
                data = resp.json()
                self.log.info(f"Open-Meteo {city['name']} keys: {list(data.keys())}")
                daily = data.get('daily') or {}
                times = daily.get('time') or daily.get('times') or []
                if times:
                    results.append({
                        'source':        'Open-Meteo',
                        'city':          city['name'],
                    ...
                    self.log.warning(f"Open-Meteo {city['name']} empty daily: {data}")
                t.sleep(1)
