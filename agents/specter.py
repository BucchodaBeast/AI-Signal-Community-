...
"""
    ...
    def _fetch_hibp(self):
        """HIBP public breach list + CISA KEV fallback.
        
        /api/v3/breaches (ALL breaches, no account lookup) is publicly accessible
        without a paid key. If it returns 401 (API policy changed), CISA KEV is
        the fallback — it's always free, always works, and is arguably higher signal
        (only actively exploited CVEs, not just exposed credentials).
        """
        try:
            resp = requests.get(
            ...
            if resp.status_code in (401, 403):
                self.log.warning("HIBP 401/403 — falling back to CISA KEV (always free)")
                return self._fetch_cisa_kev()
            resp.raise_for_status()
            breaches = resp.json()
            # Sort by AddedDate desc, filter for significant breaches only
            breaches.sort(key=lambda b: b.get('AddedDate', ''), reverse=True)
            significant = [b for b in breaches[:60] if b.get('PwnCount', 0) > 100000]
            if not significant:
                significant = breaches[:10]
    ...
    def _fetch_cisa_kev(self):
        """CISA Known Exploited Vulnerabilities Catalog — free, no auth, always available.
        
        Higher signal than raw HIBP: only vulnerabilities actively exploited in the wild.
        Updated daily by CISA. No rate limits. Perfect primary fallback.
        """
        try:
            resp = requests.get(
