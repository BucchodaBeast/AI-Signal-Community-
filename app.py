...
def create_signal_alert(tag, recent_posts):
    ...
    # Auto-trigger ORACLE to synthesise this alert
    import threading
    def _oracle():
        try:
            import time as t; t.sleep(2)  # brief pause to ensure DB write is committed
            full_alert = db.get_post(alert.get('id', ''))
            if full_alert:
                brief = ORACLE.synthesise(full_alert)
                if brief:
                    db.save_brief(brief)
                    log.info(f"ORACLE brief auto-generated: {brief['headline']}")
        except Exception as e:
...
def setup_scheduler():
    ...
    # ORACLE runs every 2 hours to catch any missed posts
    scheduler.add_job(lambda: ORACLE.run_on_unprocessed(db), 'interval', hours=2, id='oracle')
    scheduler.start()
    log.info("Scheduler started — all 7 agents + ORACLE active")
    return scheduler
