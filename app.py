...
def run_agent(name):
    ...
    try:
        # FIX #1: base.run() takes NO kwargs — recent_context removed
...
def create_signal_alert(tag, matching_posts, citizens=None):
    ...
    # FIX #3: Council auto-trigger throttled — only fires when 3+ posts are queued,
    # preventing token burn on every single convergence event
    import threading
    def _maybe_trigger_council():
        import time
        time.sleep(10)
        try:
            unprocessed = db.get_unprocessed_posts()
            if len(unprocessed) >= 3:
                log.info(f"Auto-triggering Council: {len(unprocessed)} unprocessed posts queued")
                COUNCIL.run_on_unprocessed(db)
            else:
                log.info(f"Council auto-trigger skipped ({len(unprocessed)}/3 threshold)")
        except Exception as e:
            log.error(f"Auto-Council trigger failed: {e}")

    threading.Thread(target=_maybe_trigger_council, daemon=True).start()
