...
"""
    ...
    def _cluster_signals(self, signals: list) -> list:
        ...
        for sig in signals:
            tags = sig.get('tags', [])
            # Supabase may return tags as a JSON string — parse it
            ...
            if not tags or not isinstance(tags, list):
                continue
            # Skip malformed first tags (e.g. '[' from bad JSON parse)
            raw_primary = tags[0] if tags else ''
            if not raw_primary or not isinstance(raw_primary, str) or raw_primary.startswith('['):
                continue
            primary = raw_primary.lower().lstrip('#')
    ...
    def _generate_scenario(self, cluster: dict, db) -> dict | None:
        ...
        response = self.think(prompt, max_retries=3)
        if not response:
    ...
    def _build_analysis_context(self, cluster, pattern_match, decay_info, gdelt_context, reg_context) -> dict:
        ...
        for s in signals[:8]:  # cap at 8 to control token count
            ...
            summaries.append(f"[{agent}] {body} {' '.join(str(t) for t in tags if t)}")
...
{{
  ...
  "tags": ["#{ctx['topic']}", "#scenario", "#cassandra"]
    ...
    def _build_post(self, parsed: dict, cluster: dict, pattern_match, decay_info, agents: list) -> dict:
        ...
        # Build structured body — guard every field against None
        def _s(v, fallback=''):
            """Safe string — never lets None into a concatenation."""
            return str(v) if v is not None else fallback

        body  = _s(parsed.get('signal_chain')) + "\n\n"
        body += f"BASE CASE ({_s(base.get('probability','?'))}%): {_s(base.get('trajectory'))}\n"
        body += f"→ 30d: {_s(base.get('timeline_30d'))}\n"
        body += f"→ 60d: {_s(base.get('timeline_60d'))}\n"
        body += f"→ 90d: {_s(base.get('timeline_90d'))}\n"
        body += f"Watch: {_s(base.get('leading_indicator'))}\n\n"
        body += f"DISRUPTIVE ({_s(disruptive.get('probability','?'))}%): {_s(disruptive.get('trajectory'))}\n"
        body += f"Trigger: {_s(disruptive.get('trigger'))}\n\n"
        body += f"BLACK SWAN ({_s(swan.get('probability','?'))}%): {_s(swan.get('trajectory'))}\n"
        body += f"Trigger: {_s(swan.get('trigger'))}\n\n"
        if parsed.get('historical_precedent'):
            body += f"Precedent: {_s(parsed.get('historical_precedent'))}"
