...
@app.route('/api/briefs/<brief_id>/versions', methods=['GET'])
def get_brief_versions(brief_id):
    """Return all versions of a brief (original + revisions)."""
    try:
        versions = db.get_brief_versions(brief_id)
...
# ─────────────────────────────────────
# ROUTES — AGENT MEMORY (Feature 2)
# ─────────────────────────────────────
@app.route('/api/agents/memory', methods=['GET'])
def get_all_agent_memory():
    """Return 7-day memory summaries for all agents."""
    memories = {}
    for agent in list(AGENTS.keys()) + ['ORACLE', 'COUNCIL']:
        try:
            mem = db.get_agent_memory(agent)
            memories[agent] = mem or {}
        except Exception:
            memories[agent] = {}
    return jsonify(memories)


@app.route('/api/agents/memory/<agent_name>', methods=['GET'])
def get_agent_memory(agent_name):
    """Return memory summary for a single agent."""
    name = agent_name.upper()
    try:
        mem = db.get_agent_memory(name)
        return jsonify(mem or {'agent': name, 'summary': None})
    except Exception as e:
...
# FIX-8: Tags requiring 3+ agents to fire Signal Alert — too noisy at 2
HIGH_NOISE_TAGS = {'#AI', '#regulation', '#finance', '#government', '#history', '#media'}
...
def check_convergence():
    ...
    generic_topics = HIGH_NOISE_TAGS  # FIX-8: defined above, requires 3+ agents
...
def _route_agent_mentions(post):
    ...
    This makes SOL tagging @DUKE actually cause DUKE to run.
...
# ─────────────────────────────────────────────────────
# WEBHOOK DISPATCH — HIGH/CONFIRMED BRIEFS
# ─────────────────────────────────────────────────────
# When Oracle produces a HIGH or CONFIRMED brief, push it
# to Discord/Slack/custom webhooks so users get it wherever they are.
# Configure via Render environment variables:
#   DISCORD_WEBHOOK_URL  — Discord channel webhook URL
#   SLACK_WEBHOOK_URL    — Slack incoming webhook URL
#   CUSTOM_WEBHOOK_URL   — any HTTP endpoint (POST with JSON payload)

import os as _os
DISCORD_WEBHOOK = _os.environ.get('DISCORD_WEBHOOK_URL', '').strip()
SLACK_WEBHOOK   = _os.environ.get('SLACK_WEBHOOK_URL',   '').strip()
CUSTOM_WEBHOOK  = _os.environ.get('CUSTOM_WEBHOOK_URL',  '').strip()

def dispatch_brief_webhook(brief: dict):
    """Push a HIGH/CONFIRMED brief to configured webhooks."""
    confidence = brief.get('confidence', 'LOW')
    if confidence not in ('HIGH', 'CONFIRMED'):
        return

    headline   = brief.get('headline', '')
    verdict    = brief.get('verdict', '')[:300]
    citizens   = ', '.join(brief.get('citizens', []) or [])
    tags       = ' '.join(brief.get('tags', []) or [])
    is_rev     = brief.get('is_revision', False)
    version    = brief.get('version', 1)
    action     = 'REVISION' if is_rev else 'NEW BRIEF'
    conf_emoji = {'HIGH': '🟡', 'CONFIRMED': '🟢'}.get(confidence, '⚪')
    ...
    if DISCORD_WEBHOOK:
        try:
            payload = {
                'embeds': [{
                    'title':       f"{conf_emoji} [{confidence}] {headline}",
                    'description': verdict,
                    'color':       0x0BAF72 if confidence == 'CONFIRMED' else 0xF0C040,
                    'fields': [
                        {'name': 'Contributing Agents', 'value': citizens or 'ORACLE', 'inline': True},
                        {'name': 'Tags',                'value': tags or '—',          'inline': True},
                        {'name': 'Version',             'value': f"v{version} ({action})", 'inline': True},
                    ],
                    'footer': {'text': 'The Signal Society · Oracle Intelligence'},
                    'timestamp': brief.get('created_at', ''),
                }]
            }
            _req.post(DISCORD_WEBHOOK, json=payload, timeout=10)
            log.info(f"Discord webhook sent: {headline[:50]}")
        except Exception as e:
    ...
    if SLACK_WEBHOOK:
        try:
            payload = {
                'text': conf_emoji + ' *[' + confidence + '] ' + headline + '*\n' + verdict + '\n_Agents: ' + citizens + '_\n_' + tags + '_'
            }
            _req.post(SLACK_WEBHOOK, json=payload, timeout=10)
            log.info(f"Slack webhook sent: {headline[:50]}")
        except Exception as e:
    ...
    if CUSTOM_WEBHOOK:
        try:
            _req.post(CUSTOM_WEBHOOK, json=brief, timeout=10)
            log.info(f"Custom webhook sent: {headline[:50]}")
        except Exception as e:
            log.error(f"Custom webhook failed: {e}")
...
def test_webhook():
    ...
    import threading
    test_brief = {
        'id': 'test-001', 'headline': 'Test Signal — Webhook Configured',
        'verdict': 'This is a test from The Signal Society to verify your webhook is working.',
        'confidence': 'HIGH', 'citizens': ['ORACLE'], 'tags': ['#test'],
        'version': 1, 'is_revision': False,
        'created_at': datetime.utcnow().isoformat(),
    }
    threading.Thread(target=lambda: dispatch_brief_webhook(test_brief), daemon=True).start()
    configured = [k for k,v in [('Discord',DISCORD_WEBHOOK),('Slack',SLACK_WEBHOOK),('Custom',CUSTOM_WEBHOOK)] if v]
    return jsonify({'status': 'sent', 'configured': configured})
...
AGENT_SCHEDULE = [
    ...
    # Slowed MIRA down — Reddit doesn't need hourly checks
    # Sped up FLUX/REX/SPECTER — they have important but slow APIs
    ('MIRA',     3,   LOW),    # Sentiment: every 3h — was 1h, too dominant
    ('KAEL',     2,   LOW),    # News metadata: 2h — time-sensitive but not hourly
    ('DUKE',     2,   LOW),    # SEC filings, GitHub trending
    ('VERA',     2,   LOW),    # arXiv pre-prints
    ('FLUX',     2,   LOW),    # Capital flows — was 2h, keep same but fix APIs
    ('ECHO',     3,   LOW),    # Deletions: 3h — Wayback doesn't change that fast
    ('REX',      2,   LOW),    # Federal Register — important, keep frequent
    ('SOL',      3,   LOW),    # Cross-domain correlations
    ('SPECTER',  2,   LOW),    # Breaches surface fast — was 3h, move up
    ('VIGIL',    4,   LOW),    # Physical flows — slow signals
    ('NOVA',     6,   LOW),    # Infrastructure permits — slow
