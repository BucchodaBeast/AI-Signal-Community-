...
def health():
    """System status — last runs, token budgets, queue depths."""
...
def trigger_agent(agent_name):
    ...
    def _run():
        try:
            # FIX #1: base.run() takes NO kwargs — recent_context removed entirely
...
def run_agent(name):
    ...
    try:
        # FIX: base.run() takes NO kwargs — recent_context removed
        ...
        # Catches: "_time is not defined", "time is not defined", etc.
        # These come from agent files that use time/datetime without importing.
...
def create_signal_alert(tag, matching_posts, citizens=None):
    ...
    # FIX #2: id assigned BEFORE save so it's available for _maybe_trigger_council
    ...
    # Signal alerts alone do NOT trigger Council.
    # Council is only triggered by qualified Town Halls (see create_town_hall).
    # This prevents Council from debating raw convergence noise.

def _town_hall_worth_debating(post_a, post_b, topic_tag):
    """
    Quality gate: decide whether a Town Hall is substantive enough to send to Council.

    Criteria (score ≥ 2 out of 4 to qualify):
      1. Both posts have bodies longer than 120 chars (not stubs)
      2. The topic tag is NOT a generic catch-all (#AI alone is noise)
      3. At least one post already has reactions (community found it interesting)
      4. Posts are from structurally divergent territory pairs (data vs. narrative, etc.)
    """
...
def create_town_hall(citizen_a, post_a, citizen_b, post_b, topic_tag):
    ...
    # ── COUNCIL GATE ──────────────────────────────────────────────────────────
    # Town Halls are the quality filter. Only substantive debates go to Council.
    # Signal Alerts are raw convergence noise — they go straight to the feed
    # but never trigger Council directly. The Town Hall's quality score is the
    # gate that ensures Council debates real disagreements, not tag collisions.
    if _town_hall_worth_debating(post_a, post_b, topic_tag):
        ...
        log.info(f"Town Hall did NOT qualify for Council (low signal): {citizen_a} vs {citizen_b} on {topic_tag}")
