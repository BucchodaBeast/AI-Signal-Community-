...
"""
    ...
    def _fetch_treasury(self):
        ...
        # Primary: Treasury FiscalData avg_interest_rates (correct endpoint)
        try:
            resp = requests.get(
            ...
            self.log.warning(f"Treasury FiscalData failed: {e}")

        # Fallback: FRED-style via St. Louis Fed public JSON (no auth required)
        try:
            ...
            if resp.ok:
                ...
                return [{
                    ...
                    'id':       f"fred-DGS10-{o.get('date','')}",
                    'date':     o.get('date', ''),
                    'security': 'US 10-Year Treasury (DGS10)',
                    'rate':     o.get('value', ''),
                } for o in obs if o.get('value') not in ('', '.')]
        except Exception as e:
            self.log.warning(f"FRED fallback failed: {e}")

        # Last resort: exchange rate API already fetched for forex, reuse it as a proxy signal
        self.log.error("Treasury yields: all sources failed")
        return []
