...
class SupabaseDB:
    ...
    def _deserialize_brief(self, brief):
        ...
        for field in ('evidence', 'action_items', 'citizens', 'tags', 'verified_findings', 'source_citations'):
        ...
        if 'is_revision' in brief:
            brief['is_revision'] = bool(brief['is_revision'])
        return brief
    ...
    def get_agent_memory(self, agent):
        """Get rolling 7-day memory summary for an agent."""
        try:
            r = self.client.table('agent_memory')                .select('*').eq('agent', agent).single().execute()
            return r.data or {}
        except Exception:
            return {}

    def save_agent_memory(self, agent, summary, raw_posts_count=0):
        """Upsert agent memory summary. Called after each agent run."""
        try:
            self.client.table('agent_memory').upsert({
                'agent':          agent,
                'summary':        summary[:500],
                'posts_analysed': raw_posts_count,
                'updated_at':     datetime.utcnow().isoformat(),
            }).execute()
        except Exception as e:
            import logging; logging.getLogger('database').error(f"save_agent_memory failed: {e}")

    # ── Feature 9: get_briefs_by_tag (alias for get_brief_by_topic) ───────────
    def get_briefs_by_tag(self, tag, limit=3):
        """Fetch recent briefs matching a tag — used for revision detection."""
        try:
            r = self.client.table('briefs')                .select('*')                .contains('tags', json.dumps([tag]))                .order('created_at', desc=True)                .limit(limit).execute()
            return [self._deserialize_brief(b) for b in (r.data or [])]
        except Exception as e:
            import logging; logging.getLogger('database').error(f"get_briefs_by_tag failed: {e}")
            return []

    def get_brief_by_topic(self, tags, limit=1):
        """Find most recent brief covering any of the given tags (used by oracle for versioning)."""
        for tag in (tags or [])[:3]:
            results = self.get_briefs_by_tag(tag, limit=limit)
            if results:
                # Only return if within 14 days
                from datetime import timedelta
                cutoff = (datetime.utcnow() - timedelta(days=14)).isoformat()
                recent = [b for b in results if (b.get('created_at') or '') >= cutoff]
                if recent:
                    return recent[0]
        return None
