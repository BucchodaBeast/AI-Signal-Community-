...
"""
    ...
    SOURCES = ['arxiv_applied', 'federal_register_ip', 'ssrn_ip', 'patentsview']
    ...
    def _fetch_patentsview(self):
        ...
        try:
            resp = requests.post(
                ...
                timeout=5,  # fail fast — blocked on Render
            )
            ...
            self.log.warning(f"PatentsView unavailable: {e} — using Google Patents RSS fallback")
            return self._fetch_patents_fallback()


    def _fetch_patents_fallback(self):
        """
        Google Patents RSS via patents.google.com — accessible on Render,
        no auth required for RSS feeds.
        Falls back to EPO/WIPO recent publications via their free RSS.
        """
        queries = [
            'artificial+intelligence', 'large+language+model', 'semiconductor',
            'quantum+computing', 'gene+therapy', 'battery+technology',
        ]
        q = random.choice(queries)
        try:
            resp = requests.get(
                f'https://patents.google.com/xhr/query?url=q%3D{q}&exp=&download=true',
                headers={'User-Agent': 'SignalSociety/1.0'},
                timeout=8,
            )
            if not resp.ok:
                return self._fetch_epo_rss()
            # Google Patents returns structured search results
            data    = resp.json() if resp.headers.get('content-type','').startswith('application') else {}
            results = data.get('results', {}).get('cluster', [])
            items   = []
            for cluster in results[:3]:
                for result in (cluster.get('result') or [])[:2]:
                    patent = result.get('patent', {})
                    pnum   = patent.get('publication_number', '')
                    if not pnum:
                        continue
                    items.append({
                        ...
                        'assignee': (patent.get('assignee') or ['Unknown'])[0],
                        'query':    q.replace('+', ' '),
                    })
            return items if items else self._fetch_epo_rss()
        except Exception as e:
            self.log.error(f'Google Patents fallback: {e}')
            return self._fetch_epo_rss()

    def _fetch_epo_rss(self):
        """EPO Espacenet RSS — European Patent Office, free, no key, reliable."""
        topics = [
            'artificial intelligence', 'machine learning', 'semiconductor',
            'battery', 'biosensor', 'autonomous', 'quantum',
        ]
        topic = random.choice(topics)
        try:
            resp = requests.get(
                'https://worldwide.espacenet.com/patent/search/family/rss',
                params={
                    'q': f'ta="{topic}"',
                    'lang': 'en',
                },
            ...
            for item in root.findall('.//item')[:6]:
                ...
                items.append({
                    ...
                    'id':       f'epo:{hash(guid) % 10000000}',
                    'number':   guid.split('/')[-1] if '/' in guid else '',
                    'title':    title,
                    'abstract': desc,
