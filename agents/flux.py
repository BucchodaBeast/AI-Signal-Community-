...
"""
    ...
    def _agent_specific_gate(self, item: dict) -> bool:
        """
        FLUX gate v2 — quiet markets are still signals.

        v1 problem: gate required >3% move AND >1.5x volume to pass.
        In a low-volatility period (e.g. BTC moving 0.8%) FLUX went silent.
        A silent FLUX is wrong — flat markets with normal volume are a
        macro signal in themselves. We don't gate out quiet markets;
        we gate out items with no data at all.

        Gate rules:
        - Must have a price OR a rate value (not zero)
        - Must have a source name
        - Reject only if content is truly empty (no signal at all)
        - FRED macro data always passes — rates don't need to be volatile
          to be worth surfacing (a flat Fed Funds rate for 6 months IS the signal)
        """
        source = item.get('source', '')
        meta   = item.get('metadata', {}) or {}

        # FRED, Treasury, CFTC — always pass. Macro data is always relevant.
        if any(s in source for s in ('FRED', 'Treasury', 'CFTC', 'Commodity')):
            return True

        # Crypto/price data — needs a real price, but NOT a volatility threshold
        price  = meta.get('price_usd',  0) or item.get('price_usd',  0) or 0
        value  = meta.get('value',      0) or item.get('value',      0) or 0
        change = meta.get('change_24h', 0) or item.get('change_24h', 0) or 0
        chg_pct = meta.get('change_pct', 0) or item.get('change_pct', 0) or 0

        # Must have a non-zero price or rate
        has_value = float(price or value or 0) > 0
        if not has_value:
            return False

        # Volume anomaly is HIGH signal even with small price move — always pass
        vol_mult = meta.get('volume_multiple', 1.0) or item.get('volume_multiple', 1.0) or 1.0
        if float(vol_mult) >= 1.5:
            return True

        # Summary field must have some content (not empty CoinGecko stub)
        summary = item.get('summary', '') or item.get('body', '') or ''
        if len(summary.strip()) < 20:
            return False

        # Everything with a real price passes — quiet market = valid signal
        return True
    ...
    def fetch_data(self):
        ...
        for src in srcs[:3]:
            ...
            if len(items) >= 8:
                break
        # FRED is always the guaranteed fallback — macro data never returns empty
        # This ensures FLUX never goes dark during quiet crypto periods
        if not items:
            items += self._fetch_fred_rates()
        # Even if crypto items exist, always include at least one FRED item
        # so FLUX covers macro context regardless of crypto volatility
        has_macro = any('FRED' in (i.get('source','') or '') for i in items)
        if not has_macro:
            fred_items = self._fetch_fred_rates()
            if fred_items:
                items.append(fred_items[0])
        return items
