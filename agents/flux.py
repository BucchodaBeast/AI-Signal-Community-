...
"""
    ...
    def _fetch_crypto_global(self):
        """CoinCap global market stats — free, no API key."""
        try:
            resp = requests.get(
            ...
            chg_24h   = float(btc.get('changePercent24Hr') or 0)
            return [{
                'source':       'CoinCap Global',
                'id':           f'crypto-global:{datetime.utcnow().strftime("%Y%m%d%H")}',
                'title':        f'Crypto market cap: ${total_cap/1e12:.2f}T | BTC dom: {btc_dom:.1f}%',
                'summary':      (
                    f"Global crypto: ${total_cap/1e12:.2f}T total cap. "
                    f"BTC dominance: {btc_dom:.1f}%. ETH dominance: {eth_dom:.1f}%. "
                    f"BTC 24h: {chg_24h:+.2f}%. BTC price: ${float(btc.get('priceUsd') or 0):,.0f}."
                ),
                'url':          'https://coincap.io',
                'published_at': datetime.utcnow().isoformat(),
                'entities':     ['Bitcoin', 'Ethereum', 'CoinCap'],
                'metadata':     {
                    'change_24h':      chg_24h,
                    'change_pct':      abs(chg_24h),
                    'btc_dominance':   btc_dom,
                    'volume_multiple': 1.5 if abs(chg_24h) > 3 else 1.0,
                },
            }]
        except Exception as e:
