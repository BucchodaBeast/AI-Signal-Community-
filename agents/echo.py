...
"""
    ...
    def fetch_data(self):
        ...
        if not items:
            items = self._fetch_github_deletions()
        return items
        ...
        for url in self.WATCH_LIST[:3]:
            try:
                ...
                if not text or text[0] != '[':
                    self.log.warning(f"Wayback non-JSON for {url}: {text[:100]}")
                    continue
                data    = resp.json()
                rows    = data[1:] if len(data) > 1 else []
                lengths = [int(r[2]) for r in rows if len(r) > 2 and r[2].isdigit()]
                if len(lengths) >= 2 and abs(lengths[0] - lengths[-1]) > 3000:
                    results.append({
                        'source':       'Wayback Machine',
                        'url':          url,
                        'length_delta': abs(lengths[0] - lengths[-1]),
    ...
    def _fetch_github_deletions(self):
        """GitHub public timeline — look for delete/revert events."""
        try:
            resp = requests.get(
            ...
            if not resp.ok:
                self.log.warning(f"GitHub events {resp.status_code}: {resp.text[:200]}")
                return []
            events  = resp.json()
            results = []
            for e in events:
                ...
                if etype == 'DeleteEvent':
                    results.append({'source': 'GitHub', 'event': 'deletion',
                                    'repo': repo, 'actor': actor, 'timestamp': ts,
                                    'ref': e.get('payload', {}).get('ref', '')})
                elif etype == 'PushEvent':
                    for c in e.get('payload', {}).get('commits', []):
                        msg = c.get('message', '').lower()
                        if any(w in msg for w in ['revert', 'hotfix', 'emergency', 'remove secret']):
                            results.append({'source': 'GitHub', 'event': 'suspicious_commit',
                                            'repo': repo, 'actor': actor, 'timestamp': ts,
            ...
            self.log.error(f"GitHub events failed: {e}")
            return []
