...
# ─────────────────────────────────────
# ROUTES — MARKET INTERPRET
# ─────────────────────────────────────
@app.route('/api/market-interpret', methods=['POST'])
def market_interpret():
    """
    Takes a signal body + agent name, returns a one-line trading implication.
    Uses llm_gateway so it respects existing Groq key rotation + token budget.
    Body: { "agent": "DUKE", "body": "signal text...", "tags": ["#SEC","#M&A"] }
    """
    try:
        from agents import llm_gateway
        data   = request.get_json(force=True) or {}
        agent  = data.get('agent', 'SIGNAL')
        body   = (data.get('body') or '').strip()[:800]  # cap input tokens
        tags   = ' '.join(data.get('tags') or [])

        if not body:
            return jsonify({'implication': None}), 400

        system = (
            "You are a senior market analyst translating intelligence signals into "
            "concise, actionable trading implications. Respond with ONE sentence only — "
            "maximum 25 words. Format: '[Bullish/Bearish/Watch] [asset/sector] — [reason]'. "
            "Be specific. No hedging filler. No preamble."
        )
        user = f"Agent: {agent}\nTags: {tags}\nSignal: {body}\n\nTrading implication:"
        ...
        return jsonify({'implication': result or 'No implication generated.'})

    except Exception as e:
        log.error(f'market_interpret error: {e}')
        return jsonify({'implication': None, 'error': str(e)}), 500
