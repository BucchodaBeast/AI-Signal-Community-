...
"""
    ...
    def _fetch_noaa_alerts(self):
        ...
        try:
            # message_type is not a valid query param — use only supported params
            resp = requests.get(
                'https://api.weather.gov/alerts/active',
                params={'status': 'actual', 'limit': 10},
                headers={'User-Agent': 'SignalSociety/1.0 (research@signalsociety.ai)', 'Accept': 'application/geo+json'},
                timeout=12,
            )
    ...
    def _fetch_openmeteo(self):
        ...
        try:
            ...
            def safe_current(raw):
                """Ensure all values in current block are JSON-safe primitives."""
            ...
            if isinstance(data, list):
                return [{
                    'source':  'Open-Meteo',
                    'id':      f"meteo-{selected[i][0].replace(' ','-')}-{stamp}",
                    'city':    selected[i][0],
                    'current': safe_current(d.get('current', {})),
                    'units':   d.get('current_units', {}),
                } for i, d in enumerate(data[:3])]
            # Single-city response
            return [{
                'source':  'Open-Meteo',
                'id':      f"meteo-{selected[0][0].replace(' ','-')}-{stamp}",
                'city':    selected[0][0],
                'current': safe_current(data.get('current', {})),
