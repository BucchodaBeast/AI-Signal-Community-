...
"""
    ...
    def fetch_data(self):
        # Try Wayback first, fall back to HN deleted/dead posts
    ...
    def _fetch_wayback_changes(self):
        """Check Wayback CDX via HTTP (not HTTPS) to avoid port 443 blocks."""
        results = []
        for url in self.WATCH_LIST[:3]:
            try:
                ...
                if len(lengths) >= 2 and abs(lengths[0] - lengths[-1]) > 2000:
                    results.append({
    ...
    def _fetch_hn_dead_posts(self):
        """Hacker News — fetch recent stories and check for dead/deleted ones."""
        try:
            ids   = requests.get('https://hacker-news.firebaseio.com/v0/newstories.json', timeout=10).json()[:50]
            items = []
            for sid in ids[:20]:
                try:
                    s = requests.get(f'https://hacker-news.firebaseio.com/v0/item/{sid}.json', timeout=8).json()
                    if s and s.get('dead') or s.get('deleted'):
                        items.append({
                            'source':  'Hacker News',
                            'id':      sid,
                    ...
                    continue
            # If no dead posts, return recent stories as signal data
            if not items:
                for sid in ids[:5]:
                    try:
                        s = requests.get(f'https://hacker-news.firebaseio.com/v0/item/{sid}.json', timeout=8).json()
