        ...
        with self.conn() as c:
            row = c.execute(
                """SELECT id FROM posts
                   ...
                   AND citizens LIKE ? AND citizens LIKE ?
                   AND tags LIKE ?
                   AND timestamp > ?""",
                (f'%{citizen_a}%', f'%{citizen_b}%', f'%{tag}%', since)
            ).fetchone()
        return row
...
class SupabaseDB:
    ...
    def get_recent_mentions(self, hours=6):
        ...
        try:
            ...
            import logging
            logging.getLogger('database').error(f"get_recent_mentions failed: {e}")
            return []

    def get_signal_alert_for_tag(self, tag):
        since = (datetime.utcnow() - timedelta(hours=24)).isoformat()
        # Use ilike text search instead of contains() — avoids Supabase JSON parse error on '#'
        safe_tag = tag.lstrip('#')
        try:
            r = (self.client.table('posts')
                 .select('id')
                 ...
                 .gte('timestamp', since)
                 .ilike('tags', f'%{safe_tag}%')
                 .execute())
            return r.data[0] if r.data else None
        except Exception as e:
            import logging
            logging.getLogger('database').error(f"get_signal_alert_for_tag failed: {e}")
            return None
    ...
    def get_town_hall_for_pair(self, citizen_a, citizen_b, tag):
        ...
        try:
            r = (self.client.table('posts')
                 .select('id, citizens, tags')
                 .eq('type', 'town_hall')
                 .gte('timestamp', since)
                 .execute())
            for row in r.data:
                citizens = str(row.get('citizens', ''))
                tags     = str(row.get('tags', ''))
                if (citizen_a in citizens and citizen_b in citizens
                        and safe_tag in tags):
                    return row
        except Exception as e:
            import logging
            logging.getLogger('database').error(f"get_town_hall_for_pair failed: {e}")
        return None
