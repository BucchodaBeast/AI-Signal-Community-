...
class SQLiteDB:
    ...
    def get_divergence_map(self):
        # Compute from real shared tags between structurally-opposed citizen pairs
        recent = self.get_recent_mentions(hours=48)
        from collections import defaultdict
        citizen_tags = defaultdict(set)
        for post in recent:
            tags = post.get('tags', [])
            if isinstance(tags, str):
                try: tags = json.loads(tags)
                except: tags = []
            citizen_tags[post.get('citizen', '')].update(tags)

        pairs = [('VERA','DUKE'), ('SOL','NOVA'), ('KAEL','MIRA'), ('ECHO','KAEL')]
        result = []
        for a, b in pairs:
            a_tags = citizen_tags.get(a, set())
            b_tags = citizen_tags.get(b, set())
            if a_tags and b_tags:
                overlap = len(a_tags & b_tags)
                total   = len(a_tags | b_tags)
                rate    = int((overlap / total) * 100) if total else 0
                result.append({'a': a, 'b': b, 'rate': rate, 'agree': rate > 40})
            else:
                # Fall back to defaults if insufficient data
                result.append({'a': a, 'b': b, 'rate': 0, 'agree': False})
        return result
...
class SupabaseDB:
    ...
    # JSON fields that must be serialised to TEXT before upserting
    JSON_FIELDS = ('tags', 'mentions', 'thread', 'positions', 'votes', 'reactions', 'citizens', 'raw_data')
    ...
    def init(self):
        print("Using Supabase — tables must exist via schema.sql in the Supabase SQL editor.")

    # ── helpers ──────────────────────────────────────

    def _serialise(self, post):
        """Ensure all JSON fields are strings before writing to Supabase TEXT columns."""
        out = {**post}
    ...
    def _deserialise(self, row):
        """Parse JSON strings back to Python objects after reading from Supabase."""
        if not row:
            return row
        out = {**row}
        for field in self.JSON_FIELDS:
            if field in out and isinstance(out[field], str):
                try: out[field] = json.loads(out[field])
                except: pass
        return out
    ...
    def save_post(self, post):
        ...
        post.setdefault('timestamp', datetime.utcnow().isoformat())
        post.setdefault('reactions', {'agree': 0, 'flag': 0, 'save': 0})
        self.client.table('posts').upsert(self._serialise(post)).execute()
        return post['id']
    ...
    def toggle_reaction(self, post_id, key, user_id):
        ...
        existing = (
            self.client.table('user_reactions')
            .select('id')
            .eq('post_id', post_id)
            .eq('user_id', user_id)
            .eq('reaction', key)
        ...
        if existing.data:
            ...
            # Toggle on — remove any previous reaction from this user, then insert
    ...
    def get_posts(self, limit=20, offset=0, post_type=None, citizen=None):
        ...
        if citizen:
            # Match solo posts (citizen column) OR collaborative posts (citizens JSON contains name)
            q = q.or_(f'citizen.eq.{citizen},citizens.like.%{citizen}%')
        return [self._deserialise(r) for r in q.execute().data]

    def get_post(self, post_id):
        r = self.client.table('posts').select('*').eq('id', post_id).single().execute()
        return self._deserialise(r.data)
    ...
    def get_weekly_stats(self):
        ...
        alerts = self.client.table('posts').select('id', count='exact').gte('timestamp', since).eq('type', 'signal_alert').execute().count or 0
        th     = self.client.table('posts').select('id', count='exact').gte('timestamp', since).eq('type', 'town_hall').execute().count or 0
        return {
            'posts_published': total,
            'signal_alerts':   alerts,
            'town_halls':      th,
    ...
    def get_citizen_stats(self):
        rows = (
            self.client.table('posts')
            .select('citizen,timestamp')
            .not_.is_('citizen', 'null')
            .execute()
            .data
        )
        from collections import defaultdict
        ...
        for r in rows:
            ...
            if r['timestamp'] > stats[c]['last_active']:
                stats[c]['last_active'] = r['timestamp']
        return list(stats.values())

    def get_divergence_map(self):
        """Compute real divergence from shared tags between structurally-opposed citizen pairs."""
        recent = self.get_recent_mentions(hours=48)
        from collections import defaultdict
        citizen_tags = defaultdict(set)
        for post in recent:
