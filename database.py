...
class SQLiteDB:
    ...
    def get_town_hall_for_pair(self, citizen_a, citizen_b, tag):
        """Check if a Town Hall for this citizen pair + tag already exists in last 24h."""
        since = (datetime.utcnow() - timedelta(hours=24)).isoformat()
        with self.conn() as c:
            row = c.execute(
...
class SupabaseDB:
    ...
    def save_post(self, post):
        post.setdefault('id', str(uuid.uuid4()))
        post.setdefault('timestamp', datetime.utcnow().isoformat())
        self.client.table('posts').upsert(post).execute()
        return post['id']
    ...
    def get_town_hall_for_pair(self, citizen_a, citizen_b, tag):
        ...
        r = (self.client.table('posts').select('id')
             ...
             .contains('tags', [tag])
             .execute())
        # Filter in Python — Supabase doesn't support multi-contains on text field easily
        for row in r.data:
            citizens = row.get('citizens', [])
