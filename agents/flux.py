...
"""
    ...
    def _agent_specific_gate(self, item: dict) -> bool:
        """
        FLUX gate: pass any item that has a real price/value and meaningful body.
        Quiet markets are valid signals — we never gate on % change alone.
        Only reject truly empty items (no value, no content).
        """
        # FRED and ExchangeRate items always pass — macro data is always relevant
        source = item.get('source', '')
        if any(s in source for s in ('FRED', 'ExchangeRate', 'Commodity')):
            return True
        # Price items need a non-zero value
        price = (item.get('price_usd') or item.get('usd_price') or
                 item.get('latest_val') or item.get('value') or 0)
        try:
        ...
        try:
            ...
            for c in coins[:8]:
                ...
                out.append({
                    ...
                    'title': f"{sym}: ${price:,.4f} ({chg:+.2f}% 24h)",
                    'body':  f"{c.get('name','')} ({sym}): ${price:,.4f}. 24h: {chg:+.2f}%. Vol: ${c.get('total_volume',0):,.0f}. MCap: ${c.get('market_cap',0):,.0f}.",
                })
            return out
    ...
    def _fetch_fred_rates(self):
        ...
        items    = []
        if True:  # single series — 3x timeout was blocking FLUX for 30s
            try:
                resp = requests.get(
                ...
                items.append({
                    ...
                    'change': change,
                    # body field required for base gate content check
                    'body': f"FRED {name}: {lv} on {ld}. Previous: {pv} on {pd_}. Change: {change:+.4f}.",
                    'title': f"FRED {name} = {lv}",
                })
            except Exception as e:
            ...
            for code, (name, unit) in commodity_map.items():
                ...
                if rate and rate > 0:
                    ...
                    items.append({
                        ...
                        'usd_price': price, 'unit': unit,
                        'date': date.today().isoformat(),
                        # body field required for base gate content check
                        'body': f"{name} ({code}): ${price:.2f} per {unit} as of {date.today().isoformat()}. Source: ExchangeRate-API.",
                        'title': f"{name}: ${price:.2f}/{unit}",
                    })
            return items
        except Exception as e:
    ...
    def _fetch_crypto_global(self):
        try:
            ...
            chg_24h = data.get('market_cap_change_percentage_24h_usd', 0) or 0
            return [{
                ...
                'body':  f"Total crypto market cap: ${cap/1e12:.2f}T. BTC dominance: {btc_dom:.1f}%. 24h change: {chg_24h:+.2f}%.",
                'title': f"Crypto market: ${cap/1e12:.2f}T ({chg_24h:+.1f}% 24h)",
            }]
        except Exception as e:
