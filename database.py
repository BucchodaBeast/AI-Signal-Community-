...
class SQLiteDB:
    ...
    def get_unprocessed_posts(self):
        """Return signal alerts and town halls that don't yet have a brief."""
        with self.conn() as c:
            processed_ids = {
                r[0] for r in
                c.execute("SELECT source_post_id FROM briefs").fetchall()
            }
            rows = c.execute(
...
class SupabaseDB:
    ...
    def get_signal_alert_for_tag(self, tag):
        ...
        safe_tag = tag.lstrip('#')
        try:
            r = (self.client.table('posts').select('id')
                 .eq('type','signal_alert').gte('timestamp', since)
                 .ilike('tags', f'%{safe_tag}%').execute())
    ...
    def has_seen_item(self, item_id):
        try:
            ...
            import logging; logging.getLogger('database').error(f"has_seen_item failed: {e}")
            return False
    ...
    def mark_item_seen(self, item_id, agent):
        try:
            ...
            import logging; logging.getLogger('database').error(f"mark_item_seen failed: {e}")
    ...
    def get_unprocessed_posts(self):
        ...
        processed_ids = {b['source_post_id'] for b in briefs}
