...
"""
    ...
    def _check_rate_limit_budget(self, estimated_tokens=1000):
...
Do not include any text outside the JSON object. No markdown fences."""
        ...
        try:
            ...
            if resp.status_code == 429:
                ...
                if _retry < 2:
                    wait = min(60 * (2 ** _retry), 300)  # Exponential backoff: 60s, 120s
                    self.log.warning(f"Rate limited by Groq. Cooling down for {wait}s...")
                    time.sleep(wait)
                    self._rate_limited = False  # Try again
                    return self.synthesise_from_council(session, _retry + 1)
                else:
                    self.log.error("Rate limit persists after retries. Stopping ORACLE run.")
                    return None
        ...
        try:
            ...
            for session in to_process:
                ...
                if brief:
                    ...
                    briefs.append(brief)
                    # Longer sleep to pace API calls
                    time.sleep(5)
                else:
                    # If synthesis failed (likely rate limit), stop this run
                    if self._rate_limited:
                        self.log.warning("Stopping ORACLE run due to rate limiting")
                        break
