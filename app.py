...
def create_signal_alert(tag, matching_posts, citizens=None):
    ...
    # Trigger Council to debate this alert (non-blocking, rate-limited)
    # Council will create a session, then ORACLE will pick it up on its next run
    import threading
    def _trigger_council():
        import time
        time.sleep(5)  # Small delay to let DB settle
        try:
            sessions = COUNCIL.run_on_unprocessed(db)
            log.info(f"Auto-triggered Council: {len(sessions)} session(s) created")
        except Exception as e:
