...
class BaseAgent:
    ...
    def _build_memory_block(self):
        """Pull this agent's own recent posts as recursive memory context.
        
        This transforms agents from stateless scanners into continuous analysts.
        When VERA finds a new paper, she sees her own last 20 findings and asks:
        does this confirm, contradict, or extend what I've already reported?
        """
        try:
            own_posts = db.get_posts(limit=20, citizen=self.name)
        except Exception:
            return ""
        if not own_posts:
            return ""
        
        # Only include actual dispatch posts, not meta-posts
        dispatches = [p for p in own_posts if p.get('type') == 'post'][:15]
        if not dispatches:
            return ""
        
        lines = [f"Your recent findings (last {len(dispatches)} posts you published):", ""]
        for p in dispatches:
            ts   = (p.get('timestamp') or '')[:10]  # date only
            body = (p.get('body') or '')[:150].replace('\n', ' ')
            raw_t = p.get('tags') or []
            tags = ' '.join(str(t) for t in raw_t if isinstance(t, str))
            lines.append(f"  [{ts}] {body}")
            if tags:
                lines.append(f"    {tags}")
        lines.append("")
        lines.append(
            "MEMORY INSTRUCTIONS: Before writing your post, check — does this new data "
            "CONFIRM a trend you've already reported? CONTRADICT a previous finding? "
            "EXTEND a pattern into new territory? If so, say so explicitly. "
            "Patterns spotted across multiple posts are MORE valuable than isolated findings."
        )
        return "\n".join(lines)
